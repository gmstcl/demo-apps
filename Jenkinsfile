...
echo $VERSION'''
    ...
    stage('Build') {
      steps {
        sh '''#!/bin/bash
docker build -t 226347592148.dkr.ecr.ap-northeast-2.amazonaws.com/demo-frontend:v1.1.0 .'''
      }
    }
    ...
    stage('Post-Build') {
      steps {
        sh '''#!/bin/bash
docker push 226347592148.dkr.ecr.ap-northeast-2.amazonaws.com/demo-frontend:v1.1.0'''
        sh '''#!/bin/bash
rm -rf * rm -rf .*'''
      }
    }
...
sed -i "s|tag:.*|tag: frontend-v$VERSION|g" frontend-skills-repo/values.yaml'''
      }
    }
